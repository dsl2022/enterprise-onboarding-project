...
- [ ] **Phase 3 — `request` engine + `authz`:** shared `Request` aggregate + state machine + transition guards + domain events (transactional outbox); 6 roles from the **app-roles (`roles`) claim** with most-privileged precedence, permission checks, ABAC ownership, **separation of duties**, impersonation (permissions from impersonated role; identity/SoD/ABAC/audit from the **real** principal). Declare app roles on the Entra app + assign test users to app roles.
