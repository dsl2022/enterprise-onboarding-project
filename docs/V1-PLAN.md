...
- [ ] **Phase 4 — `onboarding` + `registry` + app-reg provisioning** via `directory` (`Application.ReadWrite.OwnedBy`); async provisioning → client ID; secrets-as-references. Provisioning worker **polls `status=APPROVED`** → `markProvisioning` claim (one winner) → Graph **find-or-create** (keyed on request id; ref → `external_ref`) → `markProvisioned`. **Read ABAC at the controller:** the engine's `get()`/`timeline()` are unguarded — the controllers must enforce `app.read`/`access.read` scope (owners see only their own) before returning.
- [ ] **Phase 5 — `access`** (catalog, requests, my-access, removal) + group-add provisioning (`GroupMember.ReadWrite.All`).
  - **Known gaps to decide (CR candidates, not frozen):** (a) no lean "requester/employee" role — `access.request` is coupled to `APPLICATION_OWNER` (a 7th role would decouple it); (b) catalog `ROLE` = a governance/group, not a portal RBAC role — self-service elevation to a *portal* role would need provisioning to call Graph `appRoleAssignedTo`; (c) approvers can't swap roles at decision (APPROVE/REJECT/REQUEST_CHANGES only).
